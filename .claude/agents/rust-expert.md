...
This is **box3d-rust**, a strict port of the Box3D C computational geometry library:
...
1. **i64 overflow**: Box3D works with large coordinates. Multiplying two i64 values can overflow. Use `i128` intermediate calculations or wrapping arithmetic where C would.
...
3. **Path orientation**: Box3D uses positive area = counter-clockwise. Ensure orientation conventions match.
