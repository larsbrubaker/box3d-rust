...
//! - The outline scene tree, whole-recording query search index, per-selection
//!   inspector, and keyframe-policy popup from the C viewer are not ported; the
//!   browser viewer ships transport + scrubber + faithful playback rendering,
//!   which is the core the task scopes as must-work.
//!
...
use box3d_rust::body::{body_is_valid, get_body_transform};
