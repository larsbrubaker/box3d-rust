...
fn find_files_with_extensions(dir: &Path, extensions: &[&str]) -> Vec<PathBuf> {
    ...
    if let Ok(entries) = fs::read_dir(dir) {
        for entry in entries.flatten() {
            ...
            if path.is_dir() {
                // Skip target directory, hidden directories, and the C reference source
                if let Some(dir_name) = path.file_name().and_then(|n| n.to_str()) {
                    if dir_name == "target"
                        || dir_name.starts_with('.')
                        || dir_name == "box3d-cpp-reference"
    ...
    if !files_over_limit.is_empty() {
        ...
        error_message.push_str(&format!(
            "\nConsider breaking large files into smaller modules or functions.\n\
             ...
             This is especially important for the Box3D implementation which follows\n\
    ...
    for file_path in rust_files {
        ...
        assert!(
            ...
            "Rust file {} has {} lines, which exceeds the {}-line limit. \
             ...
             Box3D implementation which requires maintainable, focused code.",
            file_path.display(),
            line_count,
            MAX_FILE_LENGTH
        );
...
fn test_project_structure_health() {
    ...
    println!("📊 Box3D Rust Project Statistics:");
    ...
    // Additional check for Box3D-specific concerns
...
mod file_metrics {
        ...
        match extension {
            "rs" => format!(
                "Rust file {} ({} lines) refactoring suggestions:\n\
                 ...
                 - For Box3D: Ensure each module handles a single geometric operation\n\
            ...
            "py" => format!(
                "Python file {} ({} lines) refactoring suggestions:\n\
                 ...
                 - For Box3D: Keep database analysis scripts modular",
                file_path.display(),
                line_count
            ),
            "md" => format!(
                "Markdown file {} ({} lines) refactoring suggestions:\n\
                 ...
                 - For Box3D: Separate implementation rules from status tracking",
                file_path.display(),
                line_count
            ),
            _ => format!(
    ...
    fn generate_refactoring_report() {
        ...
        if !large_files.is_empty() {
            ...
            println!("\n💡 Box3D-Specific Refactoring Guidelines:");
            ...
            println!("🎉 Box3D project maintains excellent code organization!");
        }
    }

    #[test]
    fn validate_box3d_project_structure() {
        let project_root = std::env::current_dir().unwrap();
        ...
        for file in essential_files {
            ...
            assert!(
                file_path.exists(),
                "Essential Box3D file missing: {}. This indicates project structure issues.",
                file
            );
        }
        ...
        for dir in essential_dirs {
            ...
            assert!(
                ...
                "Essential Box3D directory missing: {}. This indicates project structure issues.",
                dir
            );
        }

        println!("✅ Box3D project structure is valid");
    }
}
