//! Solve Convex contact constraints. Scalar form of `b3SolveContacts_Convex`
//! (B3_SIMD_NONE): same friction order, clamp formulas, and SymMatrix muls as
//! the wide path.
//!
...
///
/// Differs from the Mesh path mainly when `use_bias == false`:
/// rolling → twist → tangent, clamps use `max / (sqrt + ε)`, and angular
/// impulse application uses [`mul_mv_sym`] / [`mul_mv2_sym`] (wide SymMatrix).
    ...
    for contact_constraint in constraints.iter_mut() {
        ...
        for j in 0..manifold_count as usize {
            ...
            for point_index in 0..point_count as usize {
                ...
                // Convex wide path: blend speculative vs soft; mesh uses the same
                // branch structure for scalar.
                ...
                // Wide: negImpulse then new = max(old - neg, 0). Algebraically
                // same as Mesh's delta = -mass*(…) - scale*old; new = max(old+delta,0).
            ...
            // Central friction
            {
                ...
                // Wide: clamp when lenSq > max^2, scale = max / (sqrt + ε)
