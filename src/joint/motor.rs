...
//
// Unlike distance/prismatic/revolute, `b3SolveMotorJoint` takes no `useBias`
// parameter -- springs always use their own softness.
//
...
pub fn warm_start_motor_joint(base: &mut JointSim, states: &mut [BodyState]) {
    ...
    // C writes unconditionally (no dynamic-flag guard) on warm start.
